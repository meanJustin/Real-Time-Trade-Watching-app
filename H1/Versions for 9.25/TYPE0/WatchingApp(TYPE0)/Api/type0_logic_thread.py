...
class External(QThread):
    ...
    def log_detail(self, detailData) :
        file_name = ENV.MASTERFILEPATH + 'Type0' + '_info.csv'
    ...
    def remove_last_row(self) :
        file = open(ENV.MASTERFILEPATH + ENV.TYPE0_PRINT, "r+")
        lines = file.readlines()
        lines.pop()
        file = open(ENV.MASTERFILEPATH + ENV.TYPE0_PRINT, "w+")
    ...
    def append_list_as_row(self, data):
        ...
        file_name = ENV.MASTERFILEPATH + 'Type0' + '.csv'
    ...
    def monitorIndexValues(self) :
        ...
        # get the current TVA which we have to compare with previous
        cur_TVA = int(cur_time.minute / 15)

        # if the current TVA is 4 then we should get the proper current line and previous line
        if cur_TVA == 0 :
            cur_TVA = 4
            cur_row = data['last_row']
            past_row = data['previous_row']
        ...
        if self.last_print == "B" :
            ...
            if cur_row[15 + cur_TVA] > past_row[15 + cur_TVA]:
                self.printBSC("DO NOTHING", cur_row, past_row)
                return
